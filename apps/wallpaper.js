...
import { buildPreviewHtml, fetchWallpaperBuffer, getWallpaperOriginalUrls, getWallpaperPage } from '../models/wallpaper.js'
...
export class Wallpaper extends plugin {
  ...
  async list (e) {
    ...
    try {
      ...
      logger.error('[chatgpt-plugin] 壁纸预览截图失败：当前环境没有可用渲染器')
      await e.reply('壁纸预览生成失败，请稍后重试')
    } catch (err) {
